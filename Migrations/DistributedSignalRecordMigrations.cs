        ...
        public int Create()
        {
            SchemaBuilder.CreateTable(typeof(DistributedEventRecord).Name,
                table => table
                    ...
                    .Column<string>("EnvironmentIdentifier", column => column.WithLength(255).NotNull()) // Should be max 63 characters, but who knows (http://stackoverflow.com/a/4097324/220230)
                    .Column<string>("Context", column => column.Unlimited())
                ).AlterTable(typeof(DistributedEventRecord).Name,
                table => table
                    .CreateIndex("EventsForIdentifier", "Id", "EnvironmentIdentifier")
                );
